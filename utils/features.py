...
def custom_features(song_path):
    features = pd.Series(index=columns(), dtype=np.float32, name=song_path)
    ...
    try:
        x, sr = librosa.load(song_path, sr=None, mono=True)  # kaiser_fast
